...
class PersonTrackingSystem:
    ...
    def __init__(self, conf_threshold=None, no_servo=False):
        ...
        self.show_angle_chart = True  # 显示角度分布图
    ...
    def _init_modules(self):
        ...
        try:
            ...
            # 5. 窗口设置
            self.window_name = "Person Tracking System - Enhanced"
    ...
    def draw_dataset_info(self, frame):
        ...
        # 获取数据集信息
        dataset = self.detector.smart_tracker.feature_library
        y_offset = panel_y + 55

        # 特征库中的角度数
        total_samples = len(dataset)
        cv2.putText(frame, f"Angle templates: {total_samples}",
                    (panel_x + 10, y_offset),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        y_offset += 25

        # 追踪数据统计
        tracking_data = self.detector.smart_tracker.tracking_data
        total_frames = tracking_data['frame_count']
        cv2.putText(frame, f"Tracked frames: {total_frames}",
                    (panel_x + 10, y_offset),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        y_offset += 25

        # 角度统计
        if tracking_data['angle_stats']:
            cv2.putText(frame, "Angle distribution:",
                        (panel_x + 10, y_offset),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
            y_offset += 20

            for angle, count in list(tracking_data['angle_stats'].items())[:3]:
                cv2.putText(frame, f"  {angle}: {count}",
                            (panel_x + 10, y_offset),
    ...
    def cleanup(self):
        ...
        # 打印最终追踪摘要
        if self.detector and self.detector.is_selecting_mode:
            summary = self.detector.get_tracking_summary()
            print("\n" + summary)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='树莓派人物追踪系统 - 增强版')
